...
function SubmissionEnergyBar(domSelector)
{
  ...
  if (element == null)
  {
    throw new Error("Unable to find submission energy bar with selector: "
      + domSelector);
  }
  if (element.dataset.serverTime == undefined)
  {
	throw new Error("Unable to find server time (used for calculations) for "
      + "bar with selector: " + domSelector);
  }

  var timeRemainingElement = element.querySelector(".time-remaining");
  if (timeRemainingElement == null)
  {
    throw new Error("Unable to find the element to output time remaining for "
      + "energy bar with selector: " + domSelector);
  }
  ...
  // If server epoch time is more than five minutes off local epoch time...
  ...
  function start()
  {
    element.querySelectorAll(".energy-slot").forEach(function(slot)
    {
      var expiration = (slot.dataset.expiration == 0)
        ? 0
        : slot.dataset.expiration - now;
  ...
  function notifyUserOfTimeMisconfiguration()
  {
    ...
    alert("It appears that there is a misconfiguration between your machine's "
      + "configured time and timezone.\n\n"
      + "This often appears when your machine is configured for one timezone, "
      + "but then the clock is adjusted to appear as in another.  For "
      + "example, the machine may be configured to use PST, but the clock was "
      + "adjusted three hours ahead to align with EST.\n\n"
      + "To fix this issue, we recommend realigning your machine's clock with "
      + "the configured timezone (or enable syncing with a time server). "
      + "Then, adjust the timezone as desired.\n\n"
...
//===========================================================================
// Feedback sections/detail links
//===========================================================================
...
function showDetailPanel(event)
{
  ...
  // Remember to blur() the button after calling this method!
  ...
  details = document.getElementsByClassName(section + " detail");
  for (var i = 0; i < details.length; i++)
  {
    var detail = details[i];
    ...
    if (detail.classList.contains('more-info'))
    {
      ...
      var height = detail.scrollHeight + 'px'; // Get it's height
      detail.style.display = ''; //  Hide it again
      detail.classList.add('visible'); // Make the element visible
      detail.style.height = height; // Update the max-height
...
{
  // Attach event handler to click event for "More..." buttons in each section
  var moreButtons = document.getElementsByClassName("more btn");
  for (var i = 0; i < moreButtons.length; i++)
  {
    moreButtons[i].addEventListener(
      'click', showDetailPanel.bind(moreButtons[i]), false);
  }
});
...
function WebAvatar() {
	...
	this.createBox = function() {
		...
		var html =
			"<style>\n"
				+ "." + this.prefix + "avatarbox { z-index:152;margin:2px;" + border + " }\n"
		...
		box.innerHTML = html;
		document.getElementById("avatar-here").appendChild(box);
...
// Maria Support
//===========================================================================
// var PHP_FILE_PATH = "http://web-cat.org/chatbot.php";
var PHP_FILE_PATH = "https://hosting.cs.vt.edu/webcat/chatbot.php";
...
function initializeMariaAvatar()
{
    SDK.applicationId = "8925636537148255758";
    web = new WebAvatar();
    web.connection = new SDKConnection();
    web.avatar = "14013806";
    web.voice = "cmu-slt";
    web.voiceMod = "default";

    web.nativeVoice = true;
    web.nativeVoiceName = "Karen";
    // Moira, Samantha, Tessa, Google US English
...
addEvent(window, 'load', function ()
{
  ...
  if (liveChat)
  {
    ...
    document.getElementById("chatbox-input").onchange = function (e)
    {
      e.preventDefault();
  ...
  // Attaching event handler to the click event for explain links for
  // individual errors
  var explainLinks = document.getElementsByClassName("explainError");
  for (var i = 0; i < explainLinks.length; i++)
  {
    explainLinks[i].addEventListener(
      'click', showPopup.bind(explainLinks[i]), false);
  }

  // Setup the bot. Send log info and signal the bot that the page has loaded.
  if (liveChat || explainLinks && explainLinks.length > 0)
  {
...
/**
 * Send a message to the bot and ignore the response. This should be used
 * to signal the bot of certain events like page load and users opening
 * the chat window.
 *
...
/**
 * Gets the name of the current user that should be sent to the bot.
 * Format: <user>__<assignment>
 *
...
function getUser()
{
  ...
  result += "__" + (course ? course.value : "null");
  var section = document.getElementById("sectionId");
  result += "__" + (section ? section.value : "null");
  result += "__" + document.getElementById("assignmentId").value;
  var userRole = document.getElementById("userRole");
  result += "__" + (userRole ? userRole.value : "viewer");
  var studentUserName = document.getElementById("studentUserName");
  if (studentUserName)
  {
    result += "__" + studentUserName.value;
  }
  return result;
}
...
/**
 * Sends information about the user to the chatbot so that it can be
 * recorded in the logs. This should be called on page load.
 */
function sendUserInfoToChatbot()
{
  ...
  var message = "setchatvars user_name is " + userName
    + " and user_first_name is " + userFirstName
    + " and user_last_name is " + userLastName
    + " and assignment_id is " + assignmentId
    + " and assignment_name is " + assignmentName
    + " and submission_number is " + submissionNumber;

  var course = document.getElementById("courseId");
  if (course)
  {
    message += " and course_id is " + course.value;
  }
  var section = document.getElementById("sectionId");
  if (section)
  {
    message += " and section_id is " + section.value;
  }
  var userRole = document.getElementById("userRole");
  if (userRole)
  {
    message += " and user_role is " + userRole.value;
  }
  var studentUserName = document.getElementById("studentUserName");
  if (studentUserName)
  {
    message += " and author_id is " + studentUserName.value;
  }
...
/**
 * Send the bot the message and updates the chat window by adding the
 *  user's message and showing the bot's response.
 *
...
function askChatbot(userText)
{
  ...
  document.getElementById("chat-history").innerHTML +=
    "<div class='chat-bubble chat-message-sent clearfix'>"
    + userText + "</div>";
...
/**
 * Send a message to the bot and process the response. This should be used
 * to chat with the bot. The responses will be added to the chat window.
 *
 * @param message The message to send to the bot.
 */
function chatWithBot(message)
{
  signalBot(message).onreadystatechange = function ()
  {
    if (this.readyState == 4 && this.status == 200)
    {
      ...
      var title = "Here's What I Found";
      var titleNode = answer.querySelector('#title');
      if (titleNode)
      {
        titleNode.remove();
        title = title.innerHTML;
      }
      document.getElementById('MariaExplanationDialog_title').innerHTML =
        title;
      ...
      {
        answer.innerHTML = "<span style=\"cursor:pointer;\">Let me see what "
      	+ "I can find....</span>";

        answer.addEventListener('click', function()
        {
          document.getElementById("modal-text").innerHTML = data.response;
          dijit.byId('MariaExplanationDialog').show();
        });
...
// Event handler for the click event on explain links which will display
// the popup
function showPopup(event)
{
  ...
  var userText = event.target.dataset.issue;
  var buttonTitle = event.target.dataset.title;
  var title = "Here's What I Found";
  ...
  document.getElementById('MariaExplanationDialog_title').innerHTML =
    title;
  xmlhttp = signalBot(userText).onreadystatechange = function ()
  {
    if (this.readyState == 4 && this.status == 200)
    {
      var data = JSON.parse(this.responseText);
