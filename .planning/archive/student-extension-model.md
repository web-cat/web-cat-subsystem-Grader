...
2. **Tier 2 (Max of Offering Hard Deadline & Student Soft Deadline):** If no student hard deadline is specified, compare the base offering's `hard_deadline` with the student extension's `soft_deadline`. Take the latest (`max`) non-nil value:
   ...
   - If the base offering has a hard deadline and the student has no soft deadline extension (or an earlier soft deadline), the offering's hard deadline is retained.
3. **Tier 3 (Base Offering Soft Deadline Fallback):** If neither an offering hard deadline nor a student soft deadline exists, fallback to the base offering's `soft_deadline`.
